...
begin
    if TG_EVENT='ddl_command_end' then
        ...
        loop
            ...
            values(TG_EVENT, TG_TAG, r.classid, r.objid, r.objsubid, r.object_type, r.schema_name, r.object_identity, r.in_extension, current_query());
         end if;
        end loop;
    end if;
     if TG_EVENT='sql_drop' then
        ...
        loop
            insert into striim.ddlcapturetable(event, tag, classid, objid, objsubid, object_type, schema_name, object_identity, is_extension, query)
            values(TG_EVENT, TG_TAG, r.classid, r.objid, r.objsubid, r.object_type, r.schema_name, r.object_identity, 'f', current_query());
        end loop;
    end if;
end;
